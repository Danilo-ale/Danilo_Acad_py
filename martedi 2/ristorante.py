...
class Clienti:
    ...
    def __init__(self, nome, cognome):      #COSTRUTTORE CLIENTE
        self.nome=nome
        self.cognome=cognome
    
    def ordina(self,menu):
        piatto=input("Inserisci piatto da ordinare: ")
        if piatto in menu:          #VERIFICA CHE IL PIATTO SIA NEL MENU
            self.ordine=Ordini(self.nome, self.cognome, piatto)         #CREA UN OGGETTO SELF.ORDINE
        else:
            print("Piatto non presente")
    def __str__(self):
        return f"Nome cliente \"{self.nome}\", cognome: \"{self.cognome}\""
    
    def ordini_cliente(self):           #VISUALIZZA CIO' CHE RITORNA IL METODO GETORDINI
...
class Ordini:           #CLASSE ORDINI, CONTIENE LISTA DEGLI ORDINI
    ...
    def getOrdini(self):        #RITORNA LA LISTA ORDINI[]
...
cliente1=Clienti(nome,cognome)      #CREAZIONE NUOVO OGGETTO CLIENTE
while True:
    opz_cliente()
    scelta=input("Inserisci scelta: ").lower()
    if scelta=="1":         #DESCRIZIONE RISTORANTE
        risto1.descrivi_ristorante()
    elif scelta=="2":       #visualizza menu
        print(risto1.getMenu())
    elif scelta=="3":           #ORDINA IL MENU
        cliente1.ordina(risto1.getMenu())
    elif scelta=="4":           #VISUALIZZA ORDINI DEL CLIENTE
